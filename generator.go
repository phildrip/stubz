...
func GenerateStubCode(ifaceData *InterfaceData, opts *options.StubOptions) (string, error) {
	...
	for path := range ifaceData.Imports {
		sortedPaths = append(sortedPaths, path)
	}
	sort.Strings(sortedPaths)

	for _, path := range sortedPaths {
		name := ifaceData.Imports[path]
		var importName *ast.Ident
		// Only add name if it's different from the last part of the path
	...
	result := "// Code generated by github.com/phildrip/toe. DO NOT EDIT.\n\n" + buf.String()
